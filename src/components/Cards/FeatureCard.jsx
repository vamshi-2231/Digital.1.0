...
const FeatureCard = ({ item, onDelete, fetchCollection, onMessage }) => {
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      ...
      if (newCoverImage) {
        const imageRef = ref(storage, `featuresImages/${item.id}_${Date.now()}_${newCoverImage.name}`);
        await uploadBytes(imageRef, newCoverImage);
        coverImageUrl = await getDownloadURL(imageRef);
      }

      const docRef = doc(db, "features", item.id);
      await updateDoc(docRef, {
        ...updatedData,
        coverImageUrl
      });

      onMessage("Feature updated successfully.");
      fetchCollection();
    } catch (err) {
      console.error("Error updating feature:", err);
      onMessage("Error updating feature.");
    } finally {
      setIsLoading(false);
      setIsEditing(false);
    }
